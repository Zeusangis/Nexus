...
def calc_acl_risk_score(
    L_flex, R_flex, L_valgus, R_valgus, trunk_forward, trunk_lateral
):
    score = 0
    if L_flex < KNEE_FLEXION_THRESHOLD or R_flex < KNEE_FLEXION_THRESHOLD:
        score += 1
    if L_flex < SEVERE_STIFF_FLEXION or R_flex < SEVERE_STIFF_FLEXION:
        score += 1
    if L_valgus > KNEE_VALGUS_THRESHOLD or R_valgus > KNEE_VALGUS_THRESHOLD:
        score += 1
    if L_valgus > SEVERE_VALGUS_THRESHOLD or R_valgus > SEVERE_VALGUS_THRESHOLD:
        score += 1
...
def process_video(video_path: str):
    ...
    while True:
        ...
        trunk_forward = (
            np.degrees(
                np.arccos(
                    np.clip(
                        np.dot(trunk_yz / np.linalg.norm(trunk_yz), vertical), -1, 1
                    )
                )
            )
            if np.linalg.norm(trunk_yz) != 0
            else 0
        )
        trunk_lateral = (
            np.degrees(
                np.arccos(
                    np.clip(
                        np.dot(trunk_xy / np.linalg.norm(trunk_xy), vertical), -1, 1
                    )
                )
            )
            if np.linalg.norm(trunk_xy) != 0
            else 0
        )

        score = calc_acl_risk_score(
            L_flex, R_flex, L_valgus, R_valgus, trunk_forward, trunk_lateral
        )
        scores.append(score)
    ...
    score_out_of_10 = min(
        round((avg_score / 8.0) * 10.0, 2), 10.0
    )  # 8 = max per-frame points
    return {
